...
class PlanningDB {
  ...
  getAllPlannings() {
    return new Promise((resolve, reject) => {
      this.db.all(
        'SELECT * FROM planning WHERE deleted = 0 ORDER BY created_date DESC', 
        (err, rows) => {
          if (err) reject(err);
          resolve(rows);
        });
  ...
  static uuid(len = 8) {
    ...
    const res = [];
    for (let i = 0; i < len; i++) {
      res.push(source.charAt(Math.floor((Math.random() * (source.length + 1)))));
    }
    return res.join('');
  }
 ...
 */  
      ...
      this.db.get(
        ...
        (err, res) => {
          ...
          if (res) {
            ...
            this.db.all(
              `SELECT id, event_date, name, description, category FROM event 
  ...
  getEvent(eventId) {
    return this._getOne(
      `SELECT id, planning_uuid, event_date, name, description, category 
      FROM event WHERE id = ?`,
      eventId
    );
  }

  createEvent(planningId, eventDate, name, description, category) {
    // We expect eventDate to be a Date object.
    return this._insert(
      'INSERT INTO event(planning_uuid, event_date, name, description, category) VALUES (?,?,?,?,?)',
      [
        planningId,
        eventDate.getTime(),
        name,
  ...
  updateEvent(eventId, eventDate, name, description, category) {
    return this._update(
      'UPDATE event SET event_date = ?, name = ?, description = ?, category = ? WHERE id = ?',
      [
        eventDate.getTime(),
        name,
        description,
